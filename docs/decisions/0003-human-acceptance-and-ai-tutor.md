...
- **AI review agent OK**（`claude-pr-review.yml` または `codex-pr-review.yml` のいずれかの 5軸レビュー pass。`review:` ラベルで起動制御）
...
## Addendum: Codex PR review 並走（2026-05-06）

### Trigger

`anthropics/claude-code-action@v1` の upstream regression（[issue #1290](https://github.com/anthropics/claude-code-action/issues/1290)）で claude-* workflow が全停止。DoD「claude-pr-review.yml review 通過」が達成不能となり、全 PR が詰まる事態が発生した。upstream 復旧は不確実。

### Decision

PR review 単一ベンダー依存を解消するため、`openai/codex-action@v1` を使う `codex-pr-review.yml` を併設する。**label 選択式**で起動制御:

- `pr-auto-label.yml` が PR open 時に default で `review: claude` ラベルを付与
- `review: claude` 付与で `claude-pr-review.yml` 起動
- `review: codex` 付与で `codex-pr-review.yml` 起動
- 両方付与で両方走る（second opinion）

DoD §AI review OK は「claude OR codex のいずれか pass」へ緩和。両方 fail なら block。

### Consequences

**Positive**:
- claude が壊れても codex に切り替えで PR review が継続可能
- 重要 PR で second opinion を取れる（両方 label 付与）
- ベンダーロックイン解消

**Negative / Trade-offs**:
- workflow ファイルが増える（codex-pr-review.yml + pr-auto-label.yml）
- secret が増える（OPENAI_API_KEY を `environment: codex` に登録）
- 両方走らせるとコストが 2 倍
- 同 PR で claude と codex が異なる verdict を出した場合の調停が必要（運用上は `rework-tracker.yml` が source 別にカウント、人間判断は両方の review を読んだ上で判断）

### Rework counter

`rework-tracker.yml` は `<!-- ai-review-verdict: ... -->`（claude）と `<!-- ai-review-verdict-codex: ... -->`（codex）の両方を grep する。claude と codex が両方 major を出した場合は別ソース扱いで 2 回 increment（異なる視点の指摘は別カウントが妥当という判断）。
