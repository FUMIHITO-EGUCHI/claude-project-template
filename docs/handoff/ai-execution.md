...
GitHub Issue / PR
   ...
   └─[pull_request opened]──▶ pr-auto-label.yml   → default で `review: claude` を付与
       │
       ├─[label = review: claude]─▶ claude-pr-review.yml → 5軸レビュー（Claude）
       └─[label = review: codex] ─▶ codex-pr-review.yml  → 5軸レビュー（Codex）
       (両方 label で両方走る — second opinion)
...
- 発火: `pull_request: opened / synchronize / ready_for_review / labeled`（draft は除外）
- 起動条件: PR に `review: claude` ラベルが付いている（pr-auto-label.yml が default で付与、剥がせば skip）
...
### 3.4 `codex-pr-review.yml`

- 発火: `pull_request: opened / synchronize / ready_for_review / labeled`（draft は除外）
- 起動条件: PR に `review: codex` ラベルが付いている（人間が手動で付与）
- やること: claude 側と対称の 5軸レビュー + verdict marker（`<!-- ai-review-verdict-codex: ... -->`）
- Tool: `openai/codex-action@v1`、`OPENAI_API_KEY` を `environment: codex` の secret から読む
- 用途: claude 側が壊れている時の代替、または second opinion（両方 label で並走）
- 触らない: merge / close / approve

### 3.5 `pr-auto-label.yml`

- 発火: `pull_request: opened`
- やること: 新規 PR に `review: claude` ラベルを default 付与
- 用途: 既存運用（claude review が default）の互換維持。codex に切り替えるには `review: claude` を剥がして `review: codex` を貼る
...
| 差分レビュー | Claude (Action) or Codex (Action) | `review:` ラベルで選択。`review: claude` が default、`review: codex` で切替、両方付与で second opinion |
...
1. **AI review OK** — `claude-pr-review.yml` または `codex-pr-review.yml` のいずれかの 5軸レビュー（correctness / readability / architecture / security / performance）通過。`review:` ラベルで起動制御。両方貼って両方 fail なら block
