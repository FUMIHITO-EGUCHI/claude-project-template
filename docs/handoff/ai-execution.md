...
`claude-pr-review.yml` が PR コメント末尾に出す verdict marker（`<!-- ai-review-verdict: ... -->`）を `rework-tracker.yml` が読み、`changes-requested-major` のときに **PR とその PR が `Closes/Refs #N` で参照する Issue 全部に `rework: N` ラベルを increment**。`minor` 指摘や `approved` ではカウントしない。
